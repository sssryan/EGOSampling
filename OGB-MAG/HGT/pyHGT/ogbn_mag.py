...
parser.add_argument('--task', type=str, default='variance_reduce',
...
parser.add_argument('--sample_width', type=int, default=600,
...
def ogbn_sample(seed, samp_nodes):
    ...
    feature, times, edge_list, indxs, _ = EGOSampling(graph, \
...
def ogbn_sample1(seed, samp_nodes):
    np.random.seed(seed)
    ylabel      = torch.LongTensor(graph.y[samp_nodes])
    feature, times, edge_list, indxs, _ = EGOSampling(graph, \
                inp = {'paper': np.concatenate([samp_nodes, graph.years[samp_nodes]]).reshape(2, -1).transpose()}, \
...
def prepare_data(pool, task_type = 'train', s_idx = 0, n_batch = args.n_batch, batch_size = args.batch_size):
    ...
    if task_type == 'train':
        ...
        for i in np.arange(n_batch):
            target_papers = np.random.choice(graph.test_paper,args.batch_size,replace = False)
            p = pool.apply_async(ogbn_sample1, args=([randint(), target_papers]))
            jobs.append(p)
